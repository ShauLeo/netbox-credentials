...
class CredentialTable(NetBoxTable):
    ...
    class Meta(NetBoxTable.Meta):
        model = Credential
        fields = (
            "pk", "id", "name", "username", "password_is_set",
            "description", "assignment_count", "tags", "actions",
        )
        default_columns = (
            "name", "username", "password_is_set", "description", "assignment_count",
        )
...
class DeviceCredentialTable(NetBoxTable):
    ...
    role = tables.Column()
    tags = columns.TagColumn(url_name="plugins:netbox_credentials:devicecredential_list")
    actions = columns.ActionsColumn()

    class Meta(NetBoxTable.Meta):
        model = DeviceCredential
        fields = ("pk", "id", "credential", "device", "role", "tags", "actions")
